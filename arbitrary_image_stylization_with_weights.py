...
flags = tf.flags
flags.DEFINE_string('checkpoint', 'models/model.ckpt', 'Path to the model checkpoint.')
flags.DEFINE_string('style_images_paths', 'images/style/style01.png', 'Paths to the style images'
                    'for evaluation.')
flags.DEFINE_string('content_images_paths', 'images/testset/blABMEM01_img_10.jpg', 'Paths to the content images'
                    'for evaluation.')
flags.DEFINE_string('output_dir', 'outputs', 'Output directory.')
flags.DEFINE_integer('image_size', 1024, 'Image size.')
      ...
      for style_i, style_img_path in enumerate(style_img_list):
        ...
        for interp_i, wi in enumerate(interpolation_weights):
          ...
          image_utils.save_np_image(
              stylized_image_res,
              os.path.join(FLAGS.output_dir, '%s_%s_%.2f_%d.jpg' %
                           (content_img_name, style_img_name, wi, FLAGS.image_size)))
